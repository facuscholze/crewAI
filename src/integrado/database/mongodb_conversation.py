...
class MongoDBConversationDB:
    ...
    def get_conversation_context(self, session_id: str) -> str:
        ...
        if is_new_conversation:
            ...
            context_parts.append("\n--- HISTORIAL RECIENTE ---")
            recent_messages = messages[-8:]  # Últimos 8 mensajes (reducido para evitar rate limits)
        ...
        if not is_new_conversation:
            ...
            recent_messages_for_context = messages[-5:] if len(messages) >= 5 else messages
    ...
    def _extract_context_info(self, messages: List[Dict]) -> Dict[str, str]:
        ...
        for msg in messages:
            if msg["type"] == "human":
                ...
                if ("me llamo" in content_lower or "soy" in content_lower) and len(content_original.split()) <= 5:
                ...
                if "@" in content_original:
                    import re
                    email_match = re.search(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b', content_original)
                    if email_match:
                ...
                if (any(char.isdigit() for char in content_original) and 
                    len(content_original.replace(" ", "")) >= 8 and 
                    len(content_original) < 20 and
                    not any(word in content_lower for word in ["masajes", "tratamiento", "precio", "cita", "agendar", "quiero", "me gustaría"])):
                ...
                if not context.get("horario_preferido"):
                    if any(word in content_lower for word in ["mañana", "tarde", "noche"]):
